...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Build adventures array
    $adventures = [];
    $selected = $_POST['selectedAdventures'] ?? [];
    $schedule = $_POST['adventureSchedule'] ?? [];
    $participants = $_POST['participants'] ?? [];
    $prices = $_POST['price'] ?? [];

    foreach ($selected as $adventureId) {
        $adventures[] = [
            'type' => $adventureId,
            'date' => $schedule[$adventureId] ?? null,
            'participants' => $participants[$adventureId] ?? 1,
            'price' => $prices[$adventureId] ?? 0
        ];
    }
    $_SESSION['adventures'] = $adventures;
    ...
    header("Location: premium_travel_confirmation.php");
    exit;
} else {
